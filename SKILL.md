...
| `worktrail uninstall` | Глобальное удаление (хуки, навык, бинарь, managed-блок) |
| `worktrail index [--save] [--json]` | Сгенерировать TASKS.md — индекс всех задач |
| `worktrail context [--json]` | Определить текущую задачу из git-контекста |
| `worktrail contract init --task-id <id> --name "..." [--scope "..."] [--relates-to <id>]` | Создать контракт задачи |
| `worktrail contract show [--task-id <id>] [--json]` | Показать контракт |
| `worktrail contract update --task-id <id> [--set <k=v>] [--relates-to <id>] [--criteria-file <f>] [--verify-file <f>]` | Обновить контракт |
...
| `worktrail decision record --task-id <id> --id <did> --title "..." --rationale "..." [--resource <uri> \| --file <p>] [--lines <r>]` | Записать решение |
| `worktrail decision list --task-id <id>` | Список решений задачи |
| `worktrail spec record --task-id <id> --id <sid> --scope "..." --invariants "..." [--resource <uri> \| --file <p>]` | Зафиксировать спек |

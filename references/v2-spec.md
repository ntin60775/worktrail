...
> **Исторический документ** (rev. 2026-06-09). Разделы о верификации (VRR),
> ревью (review run/result, review_package, эксперты, профили) и доменах
> исключены из реализации v2 core — агент сам запускает тесты и проводит
> ревью, без оркестрации worktrail. Актуальные схемы: `contract.json`,
> `progress.json`, `decision.json`, `spec.json`. Остальные схемы —
> исторические.
...
Универсальный fallback. Используется когда ни один специфичный профиль
не подходит, или когда проект не требует глубокого ревью (личные заметки,
дневник, черновики, нетиповой проект).
...
2. Если профиль не удалось определить однозначно — используется `generic`
...
- Профили: code, 1c, research, generic
